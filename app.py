...
def generate_advanced_forecast(
    ...
    try:
        ...
        # Log forecasts for validation tracking
        try:
            validation_service = ForecastValidationService()
            for fc in results["forecasts"]:
                validation_service.log_forecast(
                    target_date=(
                        fc["date"]
                        if isinstance(fc["date"], str)
                        else fc["date"].strftime("%Y-%m-%d")
                    ),
                    purity=purity,
                    forecasted_price=fc["predicted_price"],
                    lower_bound=fc["lower_bound"],
                    upper_bound=fc["upper_bound"],
                    model_name=forecast.model_name,
                )
        except Exception as log_error:
            logger.warning(f"Failed to log forecast for validation: {log_error}")
    ...
    with tab9:
        ...
        try:
            ...
            with val_tab1:
                ...
                with col1:
                    ...
                    if st.button("🔍 Validate Forecast", type="primary"):
                        with st.spinner("Validating forecast against BAJUS..."):
                            result = validation_service.validate_forecast(
                                purity=purity_to_validate,
                                forecasted_price=(
                                    manual_forecast if manual_forecast > 0 else None
                                ),
                                actual_price=(
                                    manual_actual if manual_actual > 0 else None
                                ),
                            )
                            ...
                            if result:
                                ...
                                with comp_col1:
                                    st.metric(
                                        "Forecasted",
                                        f"{result.forecasted_price:,.0f} BDT",
                                    )
                                with comp_col2:
                                    st.metric(
                                        "Actual (BAJUS)",
                                        f"{result.actual_price:,.0f} BDT",
                                    )
                                with comp_col3:
                                    delta_color = (
                                        "normal"
                                ...
                                st.warning(
                                    "Could not validate forecast. Check data availability."
                                )

                with col2:
                    st.markdown("#### Quick Stats")

                    # Get current BAJUS prices
                    try:
                        scraper = GoldPriceScraper()
                ...
                history_df = validation_service.get_validation_history(
                    days=history_days
                )
            ...
            with val_tab2:
                ...
                metrics = validation_service.get_validation_metrics(days=metrics_period)
                ...
                if metrics.total_validations > 0:
                    ...
                    report = validation_service.generate_daily_report()
                    if "improvement_trend" in report:
                        ...
                        if trend_type == "improving":
                            st.success(
                                f"📈 {trend.get('message', 'Model accuracy is improving!')}"
                            )
                        elif trend_type == "declining":
                            st.warning(
                                f"📉 {trend.get('message', 'Model accuracy declining.')}"
                            )
                        else:
                            st.info(
                                trend.get(
                                    "message",
                                    "Collecting more data for trend analysis...",
                                )
                            )
                else:
                    st.info(
            ...
            with val_tab3:
                ...
                alerts_df = validation_service.get_recent_alerts(
                ...
                if not alerts_df.empty:
                    ...
                    for _, alert in alerts_df.iterrows():
                        with st.expander(
                            ...
                            st.write(
                                f"**Forecasted:** {alert['forecasted_price']:,.0f} BDT"
                            )
                            ...
                            if not alert["acknowledged"]:
                                if st.button(
                                    ...
                                    validation_service.acknowledge_alert(alert["id"])
                                    st.rerun()
                else:
            ...
            with val_tab4:
                ...
                with model_col1:
                    ...
                    active_version = retraining_service.get_active_version()

                    if active_version:
                        st.success(f"**Version:** {active_version.version_id}")
                        st.write(f"• Created: {active_version.created_at}")
                        st.write(
                            f"• Training samples: {active_version.training_samples}"
                        )
                        st.write(
                            f"• Validation MAE: {active_version.validation_mae:.0f} BDT"
                        )
                        st.write(
                            f"• Validation MAPE: {active_version.validation_mape:.2f}%"
                        )
                    else:
                        st.info(
                            "No active model version. Train a model to get started."
                        )
                ...
                with model_col2:
                    ...
                    if schedule.get("is_enabled"):
                        st.success(
                            f"✅ Enabled ({schedule.get('frequency', 'weekly')})"
                        )
                        ...
                        if st.button("📅 Enable Weekly Retraining"):
                            retraining_service.set_retraining_schedule(
                ...
                if st.button("🔄 Retrain Models Now", type="primary"):
                    with st.spinner(
                        "Retraining models... This may take several minutes."
                    ):
                        result = retraining_service.retrain_models(
                            days=retrain_days,
                            purity=retrain_purity,
                            trigger_type="manual",
                ...
                versions = retraining_service.get_version_history(limit=5)
                        ...
                        with st.expander(
                            ...
                            if not version.is_active:
                                if st.button(
                                    ...
                                    if retraining_service.rollback_to_version(
                                        version.version_id
                                    ):
                                        st.success(
