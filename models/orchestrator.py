...
from models.deep_learning import GRUModel, LSTMModel, TCNModel
...
class ModelOrchestrator:
    ...
    def fit_all(self, X: pd.DataFrame, y: pd.Series) -> dict:
        ...
        for model_name, model in self.models.items():
            try:
                ...
                results[model_name] = {
                    "status": "success",
                    "metrics": model._diagnostics.copy() if hasattr(model, '_diagnostics') else {},
                }
    ...
    def get_ensemble_forecast(
        ...
        if len(valid_models) > self.auto_select_top_k:
            ...
            top_models = {
                name: valid_models[name] for name, _ in sorted_models[:self.auto_select_top_k]
            }
        else:
        ...
        if method == "weighted_mean":
            ...
            weights = {name: (total_var - v) / (total_var * (len(variances) - 1))
                      if total_var > 0 else 1.0 / len(variances)
        ...
        for step_idx in range(steps):
            if method == "median":
                step_values = [
                    top_models[name].predictions[step_idx]
                    for name in top_models.keys()
                ]
