...
class HybridLSTMARIMAModel(BaseTimeSeriesModel):
    ...
    def fit(
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"ARIMA fitting failed ({e}), using simple residual mean")
                self._arima_model = None
    ...
    def predict(self, steps: int = 7) -> ForecastResult:
        ...
        try:
            ...
            recent_std = np.std(combined_preds[-3:]) if len(combined_preds) >= 3 else np.std(self._training_data.values) * 0.05
